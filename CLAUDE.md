...
- **Run invalidation** — a backgrounded tab stops painting and would stall the countdown, so `visibilitychange` kills the run rather than reporting a wrong time. False starts (tapping before the target time) do the same, but are **off by default** and toggleable in settings.
