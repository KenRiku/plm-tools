...
- **Aspect-locked frame** — the run screen is a fixed overlay centring a frame locked to `1080 / 2340`, sized `min(100%, 520px, calc(100dvh * 1080 / 2340))` so it always fits. Element positions are percentages of the frame's *height* while their sizes come off its *width*, so the two only agree while the frame keeps the shape of the screen they were measured on: letting it fill an arbitrary viewport pulled the layout apart on tall windows, with buttons escaping their panel. Letterbox bars on a mismatched screen are the accepted cost of the layout staying correct. There is deliberately no fullscreen request.
- **Mock screens** — simplified but positionally faithful redraws of the three real game screens. Each element carries `--x`/`--y` (centre, as a % of the frame) and `--w`/`--h` (its size as a % of the 1080x2340 source screen), all measured off screenshots. Positions stay percentage-based so targets sit where the screen puts them, but **both** dimensions are resolved against the frame's *width* via container units — `--source-k` (2340/1080) converts a source height percentage into that width-relative unit — so a button keeps its proportions on any screen aspect. Sizing height off frame height instead made buttons ~30% flatter than the game whenever a browser toolbar shortened the viewport. `--ui-scale` scales every element and font together and is adjustable in 設定. Standard is **1**, which renders pixel-for-pixel the same as the real game (a 集結 button measures 274x70 physical px on a Galaxy S23, matching the screenshots). It is a `transform: scale()` on `.game-screen`, so positions, sizes and the gaps between them all zoom together and the layout holds its proportions exactly at any value. The game's UI already reaches the screen edges, so the ceiling is set by clipping rather than by collision: at 1.04 an outer decoy loses 0.7px and at 1.05 出征 itself loses 5px, hence `MAX_UI_SCALE` of 1.03. Any value in `MIN_UI_SCALE`-`MAX_UI_SCALE` (0.8-1.03) ranks — only the course timing has to match for a run to count — so target size varies between players on the board by design. Values outside that range only arise from hand-edited storage and are both clamped on load and rejected by `isStandardCourse()`. The frame is locked to that aspect ratio so targets land in the same place on any device. Decoy buttons are real: tapping one counts as a miss and costs time, same as in game.
...
- **Start time** — quick offsets are 5/10/20s; the picker defaults to `DEFAULT_LEAD_SECONDS` (10s) ahead and is never persisted. Targets round *up* to a whole second, so a lead is never shorter than advertised and the displayed time is exactly the moment the run arms, since a time restored from an earlier session is always in the past. A picked time that has already passed falls back to a fresh lead rather than rolling to tomorrow, and returning to the home screen refreshes a stale time while leaving a deliberate future one alone.
